...
def Align_version(frames, align, show_pic=0):
    ...
    if show_pic:
        ...
        cv2.imshow("depth", depth_image_aligned)
...
if __name__ == "__main__":
    ...
    if USE_ROS_BAG:
        config.enable_device_from_file("20230321_143201.bag")  # 这是打开相机API录制的视频
    else:
    ...
    try:
        while True:
            ...
            if key & 0xFF == ord('q') or key == 27 or i > 2400:
                cv2.destroyAllWindows()
                break
            print(i)
            #cv2.imwrite(f"rgb\\{i}.png", color_image_aligned)
            #cv2.imwrite(f"depth\\{i}.png", depth_image_aligned)
            i += 1
